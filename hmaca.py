...
class GAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi_sa, mask):
        ...
        x_grid_attn = grid_unshuffle(x_grid_attn, b, h, w, self.interval_size).view(b, h * w, c // 2)  # b h' w' c

        x_window, x_window_s = torch.split(x_window.view(b, h, w, c * 3 // 2), c * 3 // 4, dim=-1)
        x_window = window_partition(x_window, self.window_size)  # nw*b, window_size, window_size, c
        x_window = x_window.view(-1, self.window_size * self.window_size, c * 3 // 4)  # nw*b, window_size*window_size, c
        ...
        x_win_attn = self.window_attn(x_window, rpi=rpi_sa, mask=None).view(-1, self.window_size, self.window_size,
                                                                            c // 4)
        x_win_attn = window_reverse(x_win_attn, self.window_size, h, w).view(b, h * w, c // 4)  # b h' w' c

        x_win_s_attn = self.window_attn_s(x_window_s, rpi=rpi_sa, mask=mask).view(-1, self.window_size,
                                                                                  self.window_size,c // 4)
        x_win_s_attn = window_reverse(x_win_s_attn, self.window_size, h, w).view(b, h * w, c // 4)  # b h' w' c
...
# マスク画像との条件付けcross attention
class MyCrossAttention(nn.Module):
    def __init__(self, num_heads=4, in_dim=96, num_groups=32, embedding_dim=96, raw_img_size=64): # img_size渡すようにして一般化
        super().__init__()
        self.num_heads = num_heads
        self.d_model = embedding_dim
        self.d_keys = embedding_dim // num_heads
        self.d_values = embedding_dim // num_heads
        self.query_projection = nn.Linear(in_dim, embedding_dim)
        self.key_projection = nn.Linear(in_dim, embedding_dim)
        self.value_projection = nn.Linear(in_dim, embedding_dim)
        self.final_projection = nn.Linear(embedding_dim, embedding_dim)
        self.norm = nn.LayerNorm([raw_img_size*raw_img_size, embedding_dim]) # 一般化させるためにこれ絶対良くない(一般化)
    ...
    def forward(self, x, y):
        ...
        batch, hw, _ = x.shape
        # xからq, マスク画像yからk,v
        queries = self.query_projection(x)  # [b, in_dim, embedding_dim]
        keys = self.key_projection(y)       # [b, in_dim, embedding_dim]
        values = self.value_projection(y)   # [b, in_dim, embedding_dim]
        ...
        concatenated_heads_attention_scores = attention_scores.view(batch, hw, self.d_model)
...
class AttenBlocks(nn.Module):
    ...
    def __init__(self,
        ...
        # マスク画像(1, H, W)を(H*W, dim)にするのに使う
        self.my_encoder = nn.Linear(1, dim)
        # cross attention, マスク画像の条件付
        self.my_cross_attention = MyCrossAttention(raw_img_size=raw_img_size)
    ...
    def forward(self, x, x_size, params, mask):
        ...
        # Cross Attention実装
        # (batch, 1, 64, 64) -> (batch, 96, 64, 64) -> (batch, 4096, 96)
        mask = self.my_encoder(mask.view(mask.size()[0], self.raw_img_size * self.raw_img_size , 1))
        x = self.my_cross_attention(x, mask)
        return x
...
class RHTB(nn.Module):
    ...
    def forward(self, x, x_size, params, mask):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size, params, mask), x_size))) + x
...
class PartialHMANet(nn.Module):
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(PartialHMANet, self).__init__()
        ...
        self.my_conv = nn.Conv2d(3, 3, 3, 2, 1)
    ...
    def forward(self, x, mask):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, mask)) + x
            x = self.conv_before_upsample(x)
            x = self.conv_last(self.upsample(x))
            if self.upscale == 2:
                x = self.my_conv(x) # (H, W) -> (H/2, W/2)
